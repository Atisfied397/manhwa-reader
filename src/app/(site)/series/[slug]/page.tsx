...
function SeriesPageInner({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const toggleBookmark = () => {
    ...
    let bookmarks: { slug: string; title: string; coverUrl: string; addedAt: string }[] = [];
    if (saved) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/scrape?slug=${encodeURIComponent(slug)}`)
      ...
      .then((data) => {
        ...
        setChapters(
          (data.chapters || []).map((ch: { number: number; title?: string; slug?: string; pageCount?: number }) => ({
            number: ch.number,
            title: ch.title,
        ...
        <div className="flex flex-col gap-4 sm:flex-row lg:gap-5">
          ...
          <div className="flex w-full shrink-0 flex-col gap-3 rounded-lg px-2 sm:w-[240px] sm:p-0 md:sticky md:top-[76px] md:self-start xl:w-[270px]">
            ...
            <div className="hidden sm:flex flex-col gap-2">
              <Link
                href={`/reader/${slug}/${latestChapterSlug}`}
                  ...
                  {visibleChapters.map((ch) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {ch.pageCount != null && ch.pageCount > 0 && (
