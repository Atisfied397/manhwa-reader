...
function ReaderPageInner({ params }: ReaderPageProps) {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
    fetch(
      `/api/scrape/pages?slug=${encodeURIComponent(slug)}&chapter=${encodeURIComponent(chapter)}&source=${source}`,
    )
      .then((r) => r.json())
      .then((data) => {
        if (data.error) {
  ...
  const goToNext = useCallback(() => {
    const nextUrl = `/reader/${slug}/${nextChapter}?source=${source}`;
    window.location.href = nextUrl;
  }, [slug, nextChapter, source]);

  const goToPrev = useCallback(() => {
    if (!prevChapter) return;
    const prevUrl = `/reader/${slug}/${prevChapter}?source=${source}`;
    window.location.href = prevUrl;
  }, [slug, prevChapter, source]);
  ...
  useEffect(() => {
    ...
    if (next) {
      ...
      link.rel = "prefetch";
      link.href = `/reader/${slug}/${next.slug}?source=${source}`;
    ...
    if (prev) {
      const link = document.createElement("link");
      link.rel = "prefetch";
      link.href = `/reader/${slug}/${prev.slug}?source=${source}`;
      document.head.appendChild(link);
      return () => {
        document.head.removeChild(link);
      };
    }
  }, [chapterList, chapterNum, slug, source]);

  // Progressive page loading via Intersection Observer
  useEffect(() => {
    if (pages.length === 0 || visiblePages >= pages.length) return;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0]?.isIntersecting && !loadingMore) {
          setLoadingMore(true);
          // Simulate async batch load with a tiny delay for smooth UX
          setTimeout(() => {
            setVisiblePages((prev) => Math.min(prev + PAGES_PER_BATCH, pages.length));
  ...
  }, [pages, visiblePages, loadingMore]);
    ...
    >
      <div className="mx-auto max-w-4xl px-4 py-6">
        ...
        <div className="flex items-center justify-center gap-4 mb-4">
          {prevChapter ? (
            <Link
              href={`/reader/${slug}/${prevChapter}?source=${source}`}
          ...
          <Link
            href={`/series/${slug}?source=${source}`}
            className="flex items-center gap-2 rounded-lg bg-white/10 px-6 py-2.5 text-sm font-medium text-white transition-colors hover:bg-white/20"
          >
            <svg
              className="h-4 w-4"
          ...
          <Link
            href={`/reader/${slug}/${nextChapter}?source=${source}`}
        ...
        {showConfig && (
          <div className="mb-6 rounded-lg bg-white/5 p-4 space-y-3">
            ...
            <div className="text-xs text-white/40">
              Tip: Use ← → arrow keys to navigate between pages. Pages load progressively as you scroll.
            </div>
          </div>
        )}
          ...
          >
            {pages.slice(0, visiblePages).map((url, i) => (
              <img
                ...
                src={url}
                alt={`Page ${i + 1}`}
                className={`w-full ${imageFitClass} max-h-[calc(100vh-120px)]`}
