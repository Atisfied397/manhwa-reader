...
function SearchResults() {
  ...
  const highlightMatch = (text: string, query: string) => {
    ...
    return parts.map((part, i) =>
      regex.test(part) ? (
        <mark key={i} className="rounded bg-primary/30 text-primary">
          {part}
        </mark>
      ) : (
        part
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <h1 className="mb-8 text-2xl font-bold text-white">
        {debouncedInput.trim()
          ? `Search results for "${debouncedInput}"`
          : "Search Comics"}
      </h1>
      ...
      {loading ? (
            ...
            >
              ...
              <div className="flex flex-col gap-1 p-2.5">
                <h3 className="line-clamp-2 text-sm font-medium leading-tight text-foreground group-hover:text-primary">
                  {highlightMatch(comic.title, debouncedInput)}
                </h3>
                {comic.type && (
        ...
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            No results found for &ldquo;{debouncedInput}&rdquo;
          </p>
          <p className="max-w-xs text-xs text-muted-foreground">
            Try searching for a different term. Check spelling or use fewer
            keywords.
          </p>
        </div>
      ) : (
