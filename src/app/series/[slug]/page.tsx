...
export default function SeriesPage({ params }: { params: Promise<{ slug: string }> }) {
  return (
    <Suspense
      fallback={
        <div className="mx-auto max-w-[1400px] px-4 py-12 text-center text-muted-foreground">
          Loading…
        </div>
      }
    >
...
function SeriesPageInner({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const toggleBookmark = () => {
    ...
    localStorage.setItem(`bookmark-${slug}`, String(newVal));

    // Also update the shared bookmarks list
    const saved = localStorage.getItem("bookmarks");
    let bookmarks: { slug: string; title: string; coverUrl: string; source?: string; addedAt: string }[] = [];
    if (saved) {
      try { bookmarks = JSON.parse(saved); } catch { /* ignore */ }
    }
    if (newVal) {
  ...
  // Latest chapter is the first in the sorted (descending) list
  const latestChapter = chapters.length > 0 ? chapters[0] : null;
  const latestChapterSlug = latestChapter ? latestChapter.slug : "chapter-1";

  // Determine which chapters are "new" (released within last 7 days)
  const isNewChapter = (ch: ChapterInfo): boolean => {
    if (!ch.time) return false;
    const timeStr = ch.time.toLowerCase();
    if (timeStr.includes("new")) return true;
    // Check for recent time references
    return /(today|just now|1 day|2 days|3 days|4 days|5 days|6 days)/i.test(timeStr);
  };
        ...
        <div className="flex flex-col gap-4 sm:flex-row lg:gap-5">
          ...
          <div className="flex w-full shrink-0 flex-col gap-3 rounded-lg px-2 sm:w-[240px] sm:p-0 md:sticky md:top-[76px] md:self-start xl:w-[270px]">
            ...
            <div className="hidden sm:flex flex-col gap-2">
              <Link
                href={`/reader/${slug}/${latestChapterSlug}?source=${source}`}
              ...
              >
                {chapters.length > 0 ? "Read Latest" : "No Chapters"}
              </Link>
              <button
            ...
            {showBookmarkNotice && (
              <div className="hidden sm:block rounded-lg bg-card p-3 text-xs text-muted-foreground">
                <div className="flex items-start justify-between gap-2">
                  <p>
                    Bookmark this series and we&apos;ll ping you when new
                    chapters drop!
                  </p>
                  <button
                    onClick={() => setShowBookmarkNotice(false)}
            ...
            {series.genres && series.genres.length > 0 && (
              <div className="hidden sm:flex flex-wrap gap-2">
                {series.genres.map((genre) => (
                  <span
                    key={genre}
                    className="rounded border border-white/10 bg-white/5 px-2.5 py-1 text-xs font-medium text-muted-foreground"
                  >
                    {genre}
                  </span>
                ))}
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3 px-2 py-4 sm:px-3">
            ...
            {activeTab === "chapters" && (
              <div className="space-y-4">
                ...
                >
                  <div
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${allRead ? "bg-primary" : "bg-muted"}`}
                  >
                    {allRead && (
                      <svg
                ...
                {filteredChapters.length > 30 && (
                  ...
                  >
                    <span>
                      {showAllChapters
                        ? "Show less"
                        : `Show more (${filteredChapters.length - 30} more)`}
                    </span>
                    <svg
                      className={`h-3.5 w-3.5 transition-transform ${showAllChapters ? "rotate-180" : ""}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
